...
export async function POST(request: Request) {
  try {
    ...
    const { address, timezoneOffsetMinutes } = await request.json();
    ...
    const [hyperliquidResult, arcusResult] = await Promise.allSettled([
    ...
    const arcus: VenueSource = arcusResult.status === "fulfilled"
      ...
      : unavailable();
    if (hyperliquid.unavailable && arcus.unavailable) throw new Error("Both venues unavailable");
    const report = buildMultiVenueMetrics({ hyperliquid, arcus }, offset);
